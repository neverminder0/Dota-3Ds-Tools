...
def register():
    # Регистрируем свойство Scene.models с помощью функции get_models из модуля model
    bpy.types.Scene.models = bpy.props.EnumProperty(items=get_models())
    # Можно добавить и другие свойства, например:
    setattr(bpy.types.Scene, "TU_info", bpy.props.BoolProperty(default=True))
    # Регистрируем классы аддона
    for cls in classes_texture:
        bpy.utils.register_class(cls)

def unregister():
    # Отменяем регистрацию классов
    for cls in classes_texture:
        bpy.utils.unregister_class(cls)
    # Удаляем добавленные свойства у Scene
